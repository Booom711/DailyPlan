...
每天 08:30（北京时间）自动把当天的 AI Builder 速报和训练、饮食计划一起推送到微信。
...
5. 以后每天 08:30 会自动执行，也可以在 `Actions` 页面手动重新运行。
...
AI 速报来自 follow-builders 的公开 feed，每天推送最近 24-72 小时的推文、博客和播客精选。内容由脚本自动整理，不调用大模型，所以是“中文栏目 + 英文原文片段 + 链接”的结构化形式。
